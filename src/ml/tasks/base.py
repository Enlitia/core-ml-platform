...
class BaseTaskConfig(BaseSettings, ABC):
    ...
    Attributes must be set by subclasses:
        ...
        default_model_name: Default model to use from MODEL_REGISTRY (e.g., 'positive_linear').
                           Can be overridden at runtime via CLI --model flag.
        available_models: List of valid model names for this task.
        model_params: Optional dict of model-specific parameters {model_name: {param: value}}.
    ...
    default_model_name: str  # Default model (overridable via CLI)
