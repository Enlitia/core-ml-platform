...
class RandomForestModel(BaseModel):
    ...
    def get_feature_weights(self) -> dict[str, float]:
        ...
        """
        if not hasattr(self.model, "feature_importances_") or not self.feature_names_:
        ...
        if total_importance == 0:
            # Edge case: all importances are zero
            return {name: 0.0 for name in self.feature_names_}

        # Calculate percentage contribution
        weights = {}
        for name, importance in zip(self.feature_names_, importances):
            weight_pct = (importance / total_importance) * 100
